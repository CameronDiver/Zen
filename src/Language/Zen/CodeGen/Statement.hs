...
codegenWhileStatement (SAWhileStatement predicate body) =
  mdo start <- codegenExpr predicate
      ...
      mergeBlock <- L.block `L.named` "merge"
      pure ()
codegenWhileStatement _ =
  error "While statement not passed to generator for while"
